...
const ProductManageNew = (props) => {
    ...
    let listProducts = useSelector((state) => state.admin.paginationproducts);

    console.log('listProducts: ', listProducts);

    // useEffect(() => {
    //     dispatch(actions.fetchPaginationProducts(0, 9, 'San pham')); // Thay đổi ở đây!!!
    // }, [dispatch]);
    ...
    return (
        <div className="productManage">
            ...
            <div className="addProduct">
                <div className="product-head">
                    ...
                    <div className="productTitle">
                        Sản phẩm (<small>{Object.keys(listProducts).length !== 0 && listProducts?.totalItems}</small>) (Server side Pagination)
                    </div>
                </div>
            ...
            <table className="table table-striped table-bordered table-hover">
                ...
                <tbody>
                    {Object.keys(listProducts).length !== 0 && listProducts?.products?.length > 0 ? (
            ...
            {Object.keys(listProducts).length !== 0 && listProducts?.products?.length > 0 && (
                <ReactPaginate
                    previousLabel={'<'}
                    nextLabel={'>'}
                    pageCount={listProducts.totalPages}
...
export default ProductManageNew;
