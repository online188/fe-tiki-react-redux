    ...
    return (
        <>
            <div className="pl-4 notify" onClick={() => viewNotify()}>
                <i className="far fa-bell text-white position-relative">{user && notifyUnread?.length > 0 && <span className="badge badge-pill badge-danger">{notifyUnread.length}</span>}</i>
            </div>
            ...
            {showNotify && (
                <div className="detailNotify" ref={ref}>
                    ...
                    <div className="list-notify">
                        {user && notify?.length > 0 ? (
                            notify.map((item, index) => {
                                return (
                                    <div className={`${item.status === 'N1' ? styleUnread : 'notify-read'}`} key={index} onClick={() => viewDetail(item)}>
                    ...
                    <div onClick={() => markAll()} className="markAll">
