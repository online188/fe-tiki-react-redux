        ...
        <div className="bg-light">
            ...
            <div className="Result__search container">
                <div className="resultSearch">
                    <Filter keyword={keyword} priceFrom={priceFrom} priceTo={priceTo} setPriceFrom={setPriceFrom} setPriceTo={setPriceTo} filterProduct={filterProduct} />
                        ...
                        <div className="suggest__list">
                            <div className="result py-3">
                                <h5 className="m-0">
                                    ...
                                    <span className="text-primary">`{keyword ? keyword : 'Undefine'}`</span>: {result?.length ? result.length : 0} <small>Kết quả</small>
                                </h5>
                            </div>
                                ...
                                <NavItem className="d-flex">
                                    {sortType?.length > 0 &&
                                        sortType.map((item, index) => {
                                            return (
                                                <NavLink key={index} className={`${activeTab === item.id ? 'active' : ''}`} onClick={() => sort(item)}>
                                                    {item.valueVi}
                                                </NavLink>
                                            );
                                        })}
