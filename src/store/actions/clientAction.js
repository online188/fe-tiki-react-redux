...
// Filter order by status
export const filterOrderByStatusPagination = (page, size, status) => {
    return async (dispatch) => {
        try {
            const res = await filterOrderPagination(page, size, status);
            dispatch({
                type: actionTypes.FILTER_ORDER_BY_STATUS_PAGINATION,
...
export const searchOrder = (page, size, code) => {
    return async (dispatch) => {
        try {
            const res = await searchOrderService(page, size, code);
            dispatch({
