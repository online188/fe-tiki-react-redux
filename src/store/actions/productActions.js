...
export const fetchPaginationProducts = (page, size, title) => {
    return async (dispatch, getState) => {
        try {
            let res = await getPaginationProduct(page, size, title);
            dispatch({
                type: actionTypes.FETCH_PAGINATION_PRODUCTS,
                payload: res.data.products,
            });
        } catch (e) {
            toast.error('fetch pagination product error !');
        }
    };
};
